...
struct gd32_uart
{
    ...
    uint16_t rx_pin;
    uint32_t baudrate;
    struct rt_ringbuffer *serial_rx_rb;
    struct dma_element *tx_dma_list;
    volatile uint32_t tx_dma_state; // 0:stop 1:running
    rt_err_t (*rx_indicate)(rt_size_t size, void *userdata);
    void *userdata;

    rt_mp_t tx_dma_element_pool;
    rt_mp_t tx_dma_data_pool;
...
static rt_size_t gd32_uart_rx_buf_size(const struct gd32_uart *uart)
{
#ifdef BSP_USING_UART0
...
static inline void _uart_dma_transmit(struct gd32_uart *uart, const rt_uint8_t *buffer, rt_uint32_t size)
{
...
static void dma_recv_isr(struct gd32_uart *uart)
{
    ...
    if (recv_len)
    {
        ...
        rt_serial_update_write_index(uart->serial_rx_rb, recv_len);
        if (uart->rx_indicate)
        {
...
static void usart_isr(struct gd32_uart *uart)
{
...
{
    rt_base_t level = rt_hw_interrupt_disable();
    usart_isr(&uart_obj[UART0_INDEX]);
    rt_hw_interrupt_enable(level);
}
...
{
    rt_base_t level = rt_hw_interrupt_disable();
    usart_isr(&uart_obj[UART1_INDEX]);
    rt_hw_interrupt_enable(level);
}
...
{
    rt_base_t level = rt_hw_interrupt_disable();
    usart_isr(&uart_obj[UART2_INDEX]);
    rt_hw_interrupt_enable(level);
}
...
{
    rt_base_t level = rt_hw_interrupt_disable();
    usart_isr(&uart_obj[UART3_INDEX]);
    rt_hw_interrupt_enable(level);
}
...
static void gd32_dma_config(struct gd32_uart *uart)
{
    ...
    _uart_dma_receive(uart, uart->serial_rx_rb->buffer_ptr, uart->serial_rx_rb->buffer_size);
}
...
static void inline gd32_uart_init(struct gd32_uart *uart)
{
    ...
    uart->serial_rx_rb = rt_ringbuffer_create(gd32_uart_rx_buf_size(uart));
    RT_ASSERT(uart->serial_rx_rb != RT_NULL);
...
static int _usart_init(void)
{
    ...
    {
        uart_obj[i].tx_dma_element_pool = rt_mp_create("uart_tx_elem_pool",
                                                       TX_DMA_DATA_MAX_CNT,
                                                       sizeof(struct dma_element));
        RT_ASSERT(uart_obj[i].tx_dma_element_pool != RT_NULL);
        uart_obj[i].tx_dma_data_pool = rt_mp_create("uart_tx_data_pool",
                                                    TX_DMA_DATA_MAX_CNT,
                                                    TX_DMA_DATA_MAX_SIZE);
        RT_ASSERT(uart_obj[i].tx_dma_data_pool != RT_NULL);
        gd32_uart_init(&uart_obj[i]);
    }

    return 0;
}
INIT_BOARD_EXPORT(_usart_init);
...
void *gd32_uart_get_handle(const char *name)
{
    ...
    for (i = 0; i < sizeof(uart_obj) / sizeof(uart_obj[0]); i++)
    {
        if (rt_strcmp(uart_obj[i].device_name, name) == 0)
        {
            return &uart_obj[i];
        }
    }
...
rt_err_t gd32_uart_set_rx_indicate(void *handle, rt_err_t (*rx_ind)(rt_size_t size, void *userdata), void *userdata)
{
    struct gd32_uart *uart = (struct gd32_uart *)handle;
...
rt_err_t gd32_uart_dma_send(void *handle, const rt_uint8_t *buf, rt_size_t size)
{
    struct gd32_uart *uart = (struct gd32_uart *)handle;
    struct dma_element *node;
    rt_base_t level;

    if ((uart == RT_NULL) || (buf == RT_NULL) || (size == 0))
        return -RT_ERROR;

    node = rt_mp_alloc(uart->tx_dma_element_pool, RT_WAITING_NO);
    if (node == RT_NULL)
    {
        return -RT_ERROR;
    }

    node->buffer = rt_mp_alloc(uart->tx_dma_data_pool, RT_WAITING_NO);
    if (node->buffer == RT_NULL)
    {
        rt_mp_free(node);
        return -RT_ERROR;
    }

    rt_memcpy(node->buffer, buf, size);
    node->size = size;
    node->next = NULL;
    node->prev = NULL;
...
rt_err_t gd32_uart_append_dma_send_list(void *handle, struct dma_element *element)
{
    struct gd32_uart *uart = (struct gd32_uart *)handle;
    rt_base_t level;

    if ((uart == RT_NULL) || (element == RT_NULL))
        return -RT_ERROR;
...
struct dma_element *gd32_uart_alloc_dma_element(void *handle, rt_size_t size)
{
    ...
    element = rt_mp_alloc(uart->tx_dma_element_pool, RT_WAITING_NO);
    if (element)
    {
        element->buffer = rt_mp_alloc(uart->tx_dma_data_pool, RT_WAITING_NO);
        if (element->buffer == RT_NULL)
        {
            rt_mp_free(element);
            element = RT_NULL;
        }
